...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            async AverageVoltageLine () {
                const res = await this.$store.dispatch(actionTypes.averageVoltage, { id: this.id });

                this.AverageVoltage.series[0].data = [];
                this.AverageVoltage.xAxis.data = [];
                res.forEach((i) => {
                    this.AverageVoltage.series[0].data.push(i['averageVoltage']);
                    this.AverageVoltage.xAxis.data.push(i.createTime);
                });
                ...
                this.$nextTick(() => {
                    ...
                    myChart.on('dataZoom', (event) => {
                        if (event.batch) {
                            ...
                            this.AverageVoltage.dataZoom[0].start = this.dataZoom.start;
                            this.AverageVoltage.dataZoom[0].end = this.dataZoom.end;
                        } else {
                            // 滑块缩放
                            this.dataZoom = {
                                start: event.start,
                    ...
                    myChart.setOption(this.AverageVoltage);
