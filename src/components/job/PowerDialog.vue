...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            async CurrentLine () {
                const res = await this.$store.dispatch(actionTypes.power, { id: this.id });

                this.Power.series[0].data = [];
                this.Power.xAxis.data = [];
                res.forEach((i) => {
                    this.Power.series[0].data.push(i['voltage']);
                    this.Power.xAxis.data.push(i.createTime);
                });
